"""
SlotAllocator — 平台配额比例分配器

根据平台权重计算搜索配额和 top_k 槽位分配，确保多平台搜索结果的多元化。
小红书固定 40% 权重，其余平台均分 60%。使用最大余数法保证总和恒等于 total。

三个核心方法：
- allocate()：计算各平台搜索数量配额
- allocate_top_k()：按搜索配额比例分配最终结果槽位
- redistribute()：实际结果不足时，将空余槽位重分配给有剩余的平台
